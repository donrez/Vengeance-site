...
## Project: Vengeance — Minecraft client promo site

Promo/landing site for the "Vengeance" Minecraft client (Release 1.21.11),
inspired by vesence.fun but with its own identity (crimson on near-black,
Unbounded + Manrope, Russian UI copy). Design brief in `DESIGN.md`.

Current state:
- `HomePage.tsx` — full landing: fixed nav, hero (Release 1.21.11 badge),
  6 feature cards, video placeholder section, CTA (signup / disabled
  "Скачать · скоро" button), footer with Mojang/Microsoft disclaimer.
- `variants.ts`, `Card.tsx`, `Input.tsx` retuned to the dark theme;
  `Page.tsx` header, Login/Signup restyled + translated to Russian.
- `seo.config.ts` updated (siteName "Vengeance", RU description).
- Deferred: download links / Telegram-Discord links (user hasn't provided
  URLs), video embed (no trailer URL yet), products/личный кабинет page.
- Mobile: an earlier "create mobile app" flow was interrupted mid-setup —
  `mobile/` deps were installed but `mobile/.modelence-mobile-enabled` marker
  was NOT written and screens were not ported. Mobile is not active.
